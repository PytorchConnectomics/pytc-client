...
def migrate(json_path: str, target_subdir: str = "1440/"):
    ...
    if updated_count > 0:
        try:
            ...
            print(
                f"Successfully updated {updated_count} volumes to assignee 'kanchan'."
            )
        except Exception as e:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Migrate volume assignees based on subdirectory."
    )
    parser.add_argument("json_path", help="Path to the project_manager_data.json file")
    parser.add_argument(
        "--subdir",
        default="1440/",
        help="Subdirectory to match in rel_path (default: 1440/)",
    )

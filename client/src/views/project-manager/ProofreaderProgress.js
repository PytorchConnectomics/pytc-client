...
function WeeklyThroughputChart({ throughput }) {
    ...
    return (
        <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: H }}>
            {[0, 0.5, 1].map((f) => (
                <line key={f} x1={PAD} y1={PAD / 2 + chartH * (1 - f)}
                    x2={PAD + chartW} y2={PAD / 2 + chartH * (1 - f)}
                    ...
                    <g key={d.day}>
                        <Tooltip title={`${d.day}: ${d.count} items`}>
                            <rect x={x} y={y} width={barWidth} height={barH}
                                fill="#1890ff" rx={4} opacity={0.8} />
                        </Tooltip>
                        <text x={x + barWidth / 2} y={PAD / 2 + chartH + 16}
                            textAnchor="middle" fontSize={10} fill="#8c8c8c">{d.day}</text>
                        <text x={x + barWidth / 2} y={y - 6}
                            textAnchor="middle" fontSize={9} fill="#595959" fontWeight="600">
...
function ProofreaderProgress() {
    ...
    const getStats = (key) => {
        ...
        const target = quota.mon + quota.tue + quota.wed + quota.thu + quota.fri + quota.sat + quota.sun;
        const actual = (quota.actualMon || 0) + (quota.actualTue || 0) + (quota.actualWed || 0) + 
                       (quota.actualThu || 0) + (quota.actualFri || 0) + (quota.actualSat || 0) + (quota.actualSun || 0);
    ...
    const topPerformers = useMemo(() => {
        ...
        return [...withPoints].sort((a, b) => b.weeklyPoints - a.weeklyPoints).slice(0, 3);
    }, [proofreaderData, quotaData]);

    const teamAccuracy = proofreaderData.length > 0
        ? (proofreaderData.reduce((sum, p) => sum + p.accuracy, 0) / proofreaderData.length).toFixed(1)
        : 0;
    ...
    const COLUMNS = [
        {
            ...
            render: (name, record) => (
                <Space>
                    <Avatar style={{ backgroundColor: record.avatarColor }} icon={<UserOutlined />} size="small" />
                    <div>
            ...
            render: (_, record) => {
                ...
                return (
                    <Space direction="vertical" size={0} align="end" style={{ width: "100%" }}>
                        <Text>{actual.toLocaleString()} <Text type="secondary" style={{ fontSize: 11 }}>/ {target.toLocaleString()}</Text></Text>
                        <Progress
                            percent={pct}
                            size={[60, 4]}
                            showInfo={false}
    ...
    return (
        <div style={{ padding: "0 4px" }}>
            ...
            <div style={{ marginBottom: 20, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                ...
                >
                    <Button icon={<ReloadOutlined />} danger>Reset to Defaults</Button>
                </Popconfirm>
            </div>
                            ...
                            <Card size="small" hoverable style={{ borderRadius: 8 }}>
                                <Space direction="vertical" size={12} style={{ width: "100%" }}>
                                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                                        <Avatar size={40} style={{ backgroundColor: pr.avatarColor }} icon={<UserOutlined />} />
                                    ...
                                    <div>
                                        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                                            <Text type="secondary" style={{ fontSize: 11 }}>Weekly Goal</Text>
            ...
            <Row gutter={16} style={{ marginBottom: 20 }}>
                <Col span={18}>
                    <Card size="small" title={<Text strong><ThunderboltOutlined /> Active Session Metrics</Text>}>
                        <Table dataSource={proofreaderData} columns={COLUMNS} pagination={false} size="small" />
                    </Card>
                </Col>

                <Col span={6}>
                    <Card size="small" title={<Text strong><RiseOutlined /> Top Performers</Text>} style={{ height: "100%" }}>
                        <Space direction="vertical" size={16} style={{ width: "100%" }}>
                            ...
                            <div>
                                <Text strong style={{ fontSize: 12 }}>
                                    <CheckCircleOutlined style={{ color: "#52c41a" }} /> Team Accuracy
                                </Text>
                                <div style={{ marginTop: 8 }}>
                                    <Progress percent={parseFloat(teamAccuracy)} status="active" strokeColor="#52c41a" />
            ...
            <Card size="small" title={<Text strong><ClockCircleOutlined /> Team Throughput (Last 7 Days)</Text>}>
